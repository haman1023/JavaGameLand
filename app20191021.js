...
app.post('/login',
  function(req, res, next){
    req,flash("email"); //  flush email database
...
app.get('/users/:id/edit', function(req, res){
  User.findById(req.params.id, function(err, user){
    ...
    res.render("users/edit", {
                              user: user,
                              formData: req.flash('formData')[0],
                              emailError: req.falsh('emailError')[0],
                              nicknameError: req.flash('nicknameError')[0],
                              passwordError: req.flash('passwordError')[0]
                            }
    );
  ...
  User.findById(req.params.id, req.body.user, function(err, user){
    ...
    if(req.body.user.password == user.password){
      ...
      User.findByIdAndUpdate(req.params.id, req.body.user, function(err, user){
        if(err) return res.json({success:"false", message:err});
        res.redirct('/users/' + req.params.id);
      });
    } else{
...
app.get('/posts/:id/edit', function(req, res){
  Post.findById(req.params.id,function(err, post){
    if(err) return res.json({success:false, message:ree});
...
function checkUserRegValidation(req, res, next){
  ...
  async.waterfall(
    [function(callback){
      User.findOne({email: req.body.user.email, _id: {$ne: mongoose.Types.OnjectId(req.params.id)}},
        function(err, user){
          if(user){
            isValid = false;
